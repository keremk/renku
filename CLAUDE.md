...
Tutopanda is a pnpm workspace monorepo with 5 packages:

### Published Packages
- **@tutopanda/core** (`core/`): Core library for planning, managing, and running movie generation flows. Includes blueprint loading, planning engine, manifest management, and event logging.
- **@tutopanda/cli** (`cli/`): Command-line interface for movie generation. Built with Ink (React for terminals) and MCP (Model Context Protocol) integration.
- **@tutopanda/compositions** (`compositions/`): Shared Remotion compositions and renderers for video generation. Supports both browser and Node.js environments.
- **@tutopanda/providers** (`providers/`): AI provider integrations (OpenAI, Replicate, ElevenLabs). Includes producer implementations and provider registry.
