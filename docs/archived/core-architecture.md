...
├── manifests/
│   ├── rev-0001.json      # Immutable manifest (timeline, artifact mapping, config snapshot)
│   └── rev-0002.json
...
│   └── artifacts.log      # Append-only JSONL of producer executions
├── runs/
...
- The planner consumes the latest manifest, figures out dirty artifacts, and captures the next revision’s execution plan under `runs/<revision>-plan.json`. Once the plan succeeds, a new manifest (`rev-000{n}`) points logical artifact IDs to blob hashes plus derived metadata (timeline, duration, etc.).  
...
### Asset Handling
- **Binary artifacts** (audio, video, images) are written under the content-addressed `blobs/` tree. The manifest stores their hash and metadata, so deduplication falls out naturally.  
- **Readable prompts & scripts** live alongside the artifact tree in dedicated folders (e.g. `prompts/segment/segment-0003/image.txt`). Each file is hashed individually when we append artifact events, preserving formatting while still enabling dirty detection.  
- **Derived text artifacts** (scripts, summaries) remain inline in the manifest unless they grow large—in that case we mirror the prompt pattern and drop the content into `artifacts/text/<id>.txt` while the manifest references it by hash.  
...
- The blobs/ layout is a classic content-addressed scheme: every artifact gets hashed (SHA-256), and we store the payload under
  ...
  - The filename is the full hash; that lets us deduplicate identical artifacts across runs because we never rewrite the same hash
    twice.
  - When the runner writes an artifact, it records the hash in the manifest/event log alongside the metadata so downstream consumers
    can fetch it from blobs/ using the hash.

### Benefits Over Current Plan
- No path rewriting logic: every consumer dereferences artifact IDs via the manifest, so historical snapshots remain valid even after pruning.  
...
- Content-addressable blobs dramatically reduce storage when users tweak the same artifact repeatedly (e.g. narration take 5 that matches take 4 is just another manifest entry pointing to the same hash).  
...
- Requires a manifest writer that can build the full state from dirty artifacts, so we need an in-memory model during planning; this is already implicit in the current design but needs to be formalized.  
...
1. **Gather the baseline**. Load the latest manifest (`rev-n`) and hydrate it into an in-memory view that maps every artifact ID to its stored hash, dependencies, and producing node. At the same time, cache the hashes for each input source that were current when the manifest was cut.  
...
4. **Propagate dirtiness through the DAG**. Starting from the dirty inputs, walk the producer graph in topological order. Any artifact that depends on a dirty upstream node becomes dirty itself. If an artifact already has a matching entry in `artifacts.log` with the same dependency hashes, we can mark it clean and stop propagation there—nothing upstream changed, so we can reuse the previous result.  
...
1. **Run jobs and log results**. Workers dequeue jobs layer by layer. When a job finishes, it appends a JSON line to `artifacts.log` containing the provisional revision ID, artifact ID, dependency hashes, output hash, blob reference, status, and metadata such as provider cost. Appending is the only write pattern; no existing log line is ever mutated.  
2. **Handle retries safely**. Before firing a producer, the worker checks whether `artifacts.log` already holds a successful result for the same job signature (same plan ID and dependency hashes). If it does, the worker skips the API call and reuses the stored blob—this keeps reruns idempotent and cheap.  
...
4. **Assemble the manifest**. When every job succeeds, the manifest writer streams the relevant entries from `inputs.log` and `artifacts.log`, merges them with the previous manifest state, and emits a brand-new manifest file (`manifests/rev-0003.json`). This document includes updated input hashes, the resolved timeline, and a map of artifact IDs to blob hashes.  
...
interface ArtifactEvent {
  artifactId: string;     // e.g., "segment_script_0"
  revision: RevisionId;
...
interface Manifest {
  ...
  artifacts: Record<string, { hash: string; blob?: BlobRef; inline?: string }>;
...
interface JobDescriptor {
  jobId: string;
  producer: string;
  inputs: string[];                  // artifact/input IDs required
...
interface EventLog {
  ...
  streamArtifacts(movieId: string, sinceRevision?: RevisionId): AsyncIterable<ArtifactEvent>;
...
- **CLI / local development (append-only files)**  
  ...
  - Each log (`events/inputs.log`, `events/artifacts.log`) is only ever appended to; the CLI never rewrites the whole file when it emits events.  
...
- **Server / cloud deployments (Redis + S3 archive)**  
  ...
  - When the runner commits a new manifest:  
    ...
    3. Serialize the event batch to JSONL (`events/rev-0005.inputs.jsonl`, `events/rev-0005.artifacts.jsonl`) and upload it to S3 for long-term audit.  
    4. Trim the Redis stream (or delete the key) so only post-manifest edits remain buffered.  
  - On failure we simply leave the stream untouched; retries see the existing events and can skip clean artifacts. We only purge after both the manifest write and the archival upload succeed.
...
1. **Build indegree counts**. Walk the expanded blueprint and manifest to build a graph of producer → producer dependencies (inputs that are themselves produced artifacts). Track how many incoming edges (`indegree`) each producer has once dirty detection marks it for execution.
2. **Seed the ready queue**. Any dirty producer whose indegree is zero can run immediately because all of its inputs are either raw user inputs or already satisfied artifacts. We enqueue these in a FIFO (or priority) queue.
...
- The runner appends artifact/input events via `eventLog` and stores blobs/manifests via the `StorageContext`; executors perform no raw file I/O.
