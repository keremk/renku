...
- Refer to the `cli/docs/generation-diagram.png` file for a graphical representation of the `catalog/blueprints/image-only.toml` sample blueprint and the sub blueprints it refers to: `image-generate.toml`, `image-prompt-generate.toml`, and `script-generate.toml` 
...
- We are making sure that definitions are dry and removing duplicate definitions
    - Removings `nodes` property from the [graph] table. Now all 3 node types are identified as:
        ...
        - [[artifacts]] -> Corresponds to the output Artifacts from the blueprint (node type Artifact)
...
- Once the TOML files loaded and parsed, we are proposing to use a different algorithm to figure out the cardinality in order to correctly expand the definitions into the fully expanded graph.
    - E.g.(1): { from = "ScriptGenerator.NarrationScript[i]", to = "ImagePromptGenerator[i].NarrativeText" }
        - In ScriptGenerator blueprint, NarrationScript was defined as an array type indicating that it was a 1:many relationship. I.e. Producer producing many NarrationScript artifacts.
        - We denote the connection (edge) with an index NarrationScript[i] on the `from` side of the edge.
        - The `to` side indicates there should now be n instances of ImagePromptGenerator[i].NarrativeText input nodes. So this should indicate that now we need to fanout ImagePromptGenerator instances, when expanding the graph (see below for collapsing the input and artifact nodes into one artifact node after expansion for a direct connection)
        > There should be no "input" nodes between an "artifact" and "producer". I.e. this is an illegal graph: producer -> artifact -> input -> producer. It should always be producer -> artifact ->producer 
    - E.g. (2): { from = "ImagePromptGenerator[i].ImagePrompt[j]", to = "ImageGenerator[i][j].Prompt" }
        ...
        - Then the algorithm should infer that in the expanded graph, we will have i*j instances (multidimensional array), and that requires ImageGenerator[i][j].Prompt. (See below again for collapsing the input and artifact types into one artifact type)
    - E.g. (3): { from = "ScriptGenerator.MovieSummary", to = "ImagePromptGenerator[i].OverallSummary" }
        - In this case, there is a 1:many relationship. ScriptGenerator generated only one artifact ScriptGenerator.MovieSummary. 
...
- Node naming must be unified. It follows this structure: NodeType:BlueprintNamespace.Name
    - There are only 3 types of nodes: Input, Producer, Artifact. 
- Once two subBlueprints are connected, the input of the Connecting Blueprint takes the node type and name of Connected Blueprint. This ensures that the artifacts that are created can always be located with a single ID. It also captures where it was exactly created in the graph accurately and easy to debug.
    -E.g.(1) { from = "ScriptGenerator.NarrationScript[i]", to = "ImagePromptGenerator[i].NarrativeText" }
        - ScriptGenerator.NarrationScript[i] is an Artifact type node
        - ImagePromptGenerator[i].NarrativeText is an Input type node
        - They merge into a single node in the expanded graph with the nodeId="Artifact:ScriptGenerator.NarrationScript[i]"
    ...
    -E.g.(3) { from = "ImageGenerator[i][j].SegmentImage", to = "SegmentImage[i][j]"}
        - ImageGenerator[i][j].SegmentImage is an Artifact type node.
        - SegmentImage[i][j] is an Artifact type node.
        - They merge into a single node in the expanded graph with the nodeId="Artifact:ImageGenerator[i][j].SegmentImage
    -E.g.(4) { from = "ImagePromptGenerator[i].ImagePrompt[j]", to = "ImageGenerator[i][j].Prompt" }
        - ImagePromptGenerator[i].ImagePrompt[j] is an Artifact type node.
        - ImageGenerator[i][j].Prompt is an Input type node.
        - They merge into a single node in the expanded graph with the nodeId="Artifact:ImagePromptGenerator[i].ImagePrompt[j]"
- We have been also having problems identifying the inputs to the producer implementations in the providers package. Currently there are many ways of naming things. We will have one and only one way of naming nodes (artifacts, inputs, producers)
    - Depending on how the graph is expanded (including cardinalities and the namespacing from sub blueprints), the final id of the artifacts and inputs may change.
    - We need to be able to dynamically map:
        - E.g. (1): Artifact:ImagePromptGenerator[i].ImagePrompt[j] to prompt (replicate API schema), That is how the prompt is saved in the blobs and recorded in the artifacts.log. 
            ...
            - When the graph is expanded ImageGenerator[i][j].Prompt was collapsed into ImagePromptGenerator[i].ImagePrompt[j]. So replace the Prompt in the mapping with the actual nodeId = "Artifact:ImagePromptGenerator[i].ImagePrompt[j]"
    - Remove all the convoluted lookups in the producers codebase and substitute with a pre-computed (during expansion) universal nodeId. Then in runtime, the producer will simply look up that input or artifact type node. 
