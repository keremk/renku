...
async function createInputsFixture(
	root: string,
	prompt: string,
	fileName: string,
...
describe('runExecute (edit flow)', () => {
	...
	it('updates prompts and generates a new plan revision', async () => {
		...
		const queryInputsPath = await createInputsFixture(
			root,
		...
		const editInputsPath = await createInputsFixture(
			root,
			'Tell me about stars',
			'edit-inputs.yaml'
		);
