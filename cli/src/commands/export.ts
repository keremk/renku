	...
	for (const [key, entry] of subtitleInputs) {
		...
		switch (fieldName) {
			...
			case 'backgroundOpacity':
				...
				break;
			case 'position':
				if (isOverlayPosition(value)) {
					config.position = value;
				}
				break;
			case 'edgePaddingPercent':
				if (typeof value === 'number') {
					config.edgePaddingPercent = value;
				}
				break;
...
function extractTextConfig(manifest: {
	...
	for (const [key, entry] of textInputs) {
		const fieldName = key.slice(prefix.length);
		const value = JSON.parse(entry.payloadDigest);
...
/** Known keys within the text object */
...
export async function loadExportConfig(
	...
	if (unknownKeys.length > 0) {
		throw createProviderError(
			...
			`Unknown keys in export config: [${unknownKeys.join(', ')}]\n` +
				...
				`Valid text keys are: ${[...KNOWN_TEXT_KEYS].join(', ')}`,
			{ kind: 'user_input', causedByUser: true }
		);
	}
	...
	if (parsed.subtitles !== undefined) {
		...
		config.subtitles = validateSubtitleConfig(
			parsed.subtitles as Record<string, unknown>
		);
	}
	...
	if (parsed.text !== undefined) {
		if (typeof parsed.text !== 'object' || parsed.text === null) {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text' must be an object, got ${typeof parsed.text}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.text = validateTextConfig(parsed.text as Record<string, unknown>);
	}
...
function validateSubtitleConfig(raw: Record<string, unknown>): SubtitleConfig {
	...
	if (raw.position !== undefined) {
		if (!isOverlayPosition(raw.position)) {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'subtitles.position' must be one of ${OVERLAY_POSITIONS.join(', ')}, got ${String(raw.position)}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
	...
	if (raw.edgePaddingPercent !== undefined) {
		if (typeof raw.edgePaddingPercent !== 'number') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'subtitles.edgePaddingPercent' must be a number, got ${typeof raw.edgePaddingPercent}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.edgePaddingPercent = raw.edgePaddingPercent;
	}
...
function validateTextConfig(raw: Record<string, unknown>): TextConfig {
	const config: TextConfig = {};
	...
	if (raw.font !== undefined) {
		if (typeof raw.font !== 'string') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.font' must be a string, got ${typeof raw.font}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.font = raw.font;
	...
	if (raw.fontSize !== undefined) {
		if (typeof raw.fontSize !== 'number') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.fontSize' must be a number, got ${typeof raw.fontSize}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.fontSize = raw.fontSize;
	...
	if (raw.fontBaseColor !== undefined) {
		if (typeof raw.fontBaseColor !== 'string') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.fontBaseColor' must be a string, got ${typeof raw.fontBaseColor}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.fontBaseColor = raw.fontBaseColor;
	...
	if (raw.backgroundColor !== undefined) {
		if (typeof raw.backgroundColor !== 'string') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.backgroundColor' must be a string, got ${typeof raw.backgroundColor}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.backgroundColor = raw.backgroundColor;
	...
	if (raw.backgroundOpacity !== undefined) {
		if (typeof raw.backgroundOpacity !== 'number') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.backgroundOpacity' must be a number, got ${typeof raw.backgroundOpacity}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.backgroundOpacity = raw.backgroundOpacity;
	...
	if (raw.position !== undefined) {
		if (!isOverlayPosition(raw.position)) {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.position' must be one of ${OVERLAY_POSITIONS.join(', ')}, got ${String(raw.position)}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.position = raw.position;
	...
	if (raw.edgePaddingPercent !== undefined) {
		if (typeof raw.edgePaddingPercent !== 'number') {
			throw createProviderError(
				SdkErrorCode.INVALID_CONFIG,
				`Export config: 'text.edgePaddingPercent' must be a number, got ${typeof raw.edgePaddingPercent}`,
				{ kind: 'user_input', causedByUser: true }
			);
		}
		config.edgePaddingPercent = raw.edgePaddingPercent;
...
function isOverlayPosition(value: unknown): value is OverlayPosition {
	return (
		typeof value === 'string' &&
		OVERLAY_POSITIONS.includes(value as OverlayPosition)
	);
}
...
export async function runExport(options: ExportOptions): Promise<ExportResult> {
	...
	const exporter: ExporterType =
		options.exporter ?? fileConfig?.exporter ?? 'remotion';
	...
	const exporterModel =
		exporter === 'ffmpeg' ? 'ffmpeg/native-render' : 'remotion/docker-render';
	...
	if (textConfig) {
		providerConfig.text = textConfig;
	}
...
async function validateBlueprintHasTimelineComposer(
