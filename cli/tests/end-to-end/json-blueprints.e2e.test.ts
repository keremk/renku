...
describe('end-to-end: JSON virtual artifact blueprint', () => {
	...
	it('dry-run generates correct jobs with virtual artifact connections', async () => {
		const blueprintPath = resolve(
			CLI_FIXTURES_BLUEPRINTS,
			'json-blueprints',
			'json-blueprints.yaml'
		);
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'json-blueprints-inputs.yaml'
		);
		...
		const imageJobs = allJobs.filter(
			(j: any) => j.producer === 'ImageProducer'
		);
		const timelineJobs = allJobs.filter(
	...
	it('re-runs only affected ImageProducer when virtual artifact is overridden', async () => {
		...
		const timelineJob = initialJobs.find(
			(j: any) => j.producer === 'TimelineComposer'
		);
		...
		const overridePromptContent =
			'A beautiful sunrise over the lunar landscape';
		...
		const editDocJobs = editJobs.filter(
			(j: any) => j.producer === 'DocProducer'
		);
		const editImageJobs = editJobs.filter(
			(j: any) => j.producer === 'ImageProducer'
		);
		const editTimelineJobs = editJobs.filter(
			(j: any) => j.producer === 'TimelineComposer'
		);
		...
		expect(
			allEditJobIds.some(
				(id: string) => id.includes('ImageProducer') && id.includes('[0][1]')
			)
		).toBe(false);
		expect(
			allEditJobIds.some(
				(id: string) => id.includes('ImageProducer') && id.includes('[1][0]')
			)
		).toBe(false);
		expect(
			allEditJobIds.some(
				(id: string) => id.includes('ImageProducer') && id.includes('[1][1]')
			)
		).toBe(false);
	...
	it('re-runs different ImageProducer when different virtual artifact is overridden', async () => {
		...
		await writeFile(
			overridePromptPath,
			'A dramatic shot of astronauts on the moon',
			'utf8'
		);
		...
		expect(
			allEditJobIds.some(
				(id: string) => id.includes('ImageProducer') && id.includes('[0][0]')
			)
		).toBe(false);
		expect(
			allEditJobIds.some(
