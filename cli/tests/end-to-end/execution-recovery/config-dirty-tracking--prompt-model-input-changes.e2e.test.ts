 ...
 *
 * Uses the audio-narration-loop blueprint fixture:
...
describe('end-to-end: config dirty tracking', () => {
	...
	it('TOML systemPrompt change triggers re-run + downstream propagation', async () => {
		...
		const sourceBlueprint = resolve(
			CLI_FIXTURES_BLUEPRINTS,
			'pipeline-orchestration',
			'audio-narration-loop'
		);
		const sourceSharedBlueprints = CLI_FIXTURES_BLUEPRINT_MODULES;
		const tempBlueprintDir = join(
			tempRoot,
			'pipeline-orchestration',
			'audio-narration-loop'
		);
		const tempSharedBlueprintDir = join(tempRoot, '_shared');
		await mkdir(join(tempRoot, 'pipeline-orchestration'), {
			recursive: true,
		});
		await cp(sourceBlueprint, tempBlueprintDir, { recursive: true });
		await cp(sourceSharedBlueprints, tempSharedBlueprintDir, {
			recursive: true,
		});
		...
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'audio-narration-loop--default.inputs.yaml'
		);
		...
		const scriptTomlPath = resolve(
			tempSharedBlueprintDir,
			'script',
			'script.toml'
		);
		...
		const modifiedPrompts = {
			...originalPrompts,
			systemPrompt: 'MODIFIED: You are a different writer now.',
		};
		...
		const scriptJobs = phase2Jobs.filter(
			(j: { producer: string }) => j.producer === 'ScriptProducer'
		);
		const audioJobs = phase2Jobs.filter(
		...
		if (phase2.planResult.explanation) {
			...
			expect(scriptExplanation?.dirtyInputs ?? []).toContain(
				'Input:ScriptProducer.systemPrompt'
			);
		}
	});
	...
	it('config change in input YAML triggers re-run', async () => {
		...
		const scriptJobs = phase2.allJobs.filter(
			(j: { producer: string }) => j.producer === 'ScriptProducer'
		);
		const audioJobs = phase2.allJobs.filter(
			(j: { producer: string }) => j.producer === 'AudioProducer'
		);
	...
	it('builds folder TOML overrides blueprint template', async () => {
		...
		const { saveProducerPrompts, loadPromptFile } = await import(
			'@gorenku/core'
		);

		// Load original prompts
		const originalTomlPath = resolve(
			CLI_FIXTURES_BLUEPRINT_MODULES,
			'script',
			'script.toml'
		);
		...
		await saveProducerPrompts(buildsDir, 'ScriptProducer', {
			...originalPrompts,
			systemPrompt:
				'BUILDS OVERRIDE: Different system prompt from builds folder.',
		});
