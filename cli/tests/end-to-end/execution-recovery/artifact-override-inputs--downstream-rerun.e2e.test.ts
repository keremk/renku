...
describe('end-to-end: artifact override via inputs.yaml', () => {
	...
	it('re-runs downstream producers when artifact is overridden via file: prefix', async () => {
		...
		const scriptJob = initialJobs.find(
		...
		const overrideScriptContent =
			'This is the overridden narration script for segment 0.';
		...
		const editScriptJobs = editJobs.filter(
			(j: any) => j.producer === 'ScriptProducer'
		);
		const editAudioJobs = editJobs.filter(
			(j: any) => j.producer === 'AudioProducer'
		);
