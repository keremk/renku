...
describe('end-to-end: indexed collection binding', () => {
	...
	it('wires constant-indexed collection inputs (ReferenceImages[0], ReferenceImages[1]) to VideoProducer', async () => {
		...
		const blueprintPath = resolve(
			...
			'constant-indexed-collection',
			'constant-indexed-collection.yaml'
		);
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'constant-indexed-collection--dual-reference.inputs.yaml'
		);
		...
		const characterImageJobs = plan.layers
			.flat()
			.filter((job: any) => job.producer === 'CharacterImageProducer');
		expect(characterImageJobs).toHaveLength(1);

		const productImageJobs = plan.layers
			.flat()
			.filter((job: any) => job.producer === 'ProductImageProducer');
		...
		const sortedVideos = [...videoJobs].sort(
		...
		for (const videoJob of sortedVideos) {
			...
			expect(bindings['ReferenceImages[0]']).toBe(
				'Artifact:CharacterImageProducer.GeneratedImage'
			);
			expect(bindings['ReferenceImages[1]']).toBe(
				'Artifact:ProductImageProducer.GeneratedImage'
			);
		}
