...
describe('end-to-end: multi-looped inputs dimension unification', () => {
	...
	it('generates plan for blueprint with multi-looped inputs from sibling fields', async () => {
		...
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'sibling-dimension-unification--default.inputs.yaml'
		);
		...
		const contentJobs = allJobs.filter(
			(j: any) => j.producer === 'ContentGenerator'
		);
		const imageJobs = allJobs.filter(
			(j: any) => j.producer === 'ImageGenerator'
		);
		const videoJobs = allJobs.filter(
			(j: any) => j.producer === 'VideoGenerator'
		);
	...
	it('executes plan with multi-looped inputs successfully', async () => {
		...
		expect(
			succeededJobs.filter((j) => j.producer === 'ContentGenerator')
		).toHaveLength(1);
		expect(
			succeededJobs.filter((j) => j.producer === 'ImageGenerator')
		).toHaveLength(3);
		expect(
			succeededJobs.filter((j) => j.producer === 'VideoGenerator')
		).toHaveLength(3);
		...
		const imageArtifacts = artifactIds.filter(
			(id) => id.includes('GeneratedImage') || id.includes('Image')
		);
		const videoArtifacts = artifactIds.filter(
			(id) => id.includes('FinalVideo') || id.includes('Video')
		);
