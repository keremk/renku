...
describe('end-to-end: blob inputs via file: prefix', () => {
				...
				models: [
					{
						...
						provider: 'fal-ai',
						producerId: 'TestImageToVideoProducer',
					},
				],
			}),
			'utf8'
	...
	it('resolves multiple file: references in an array input', async () => {
		...
		await writeFile(
			...
			stringifyYaml({
				inputs: {
					...
					InputImages: [`file:${testImagePath1}`, `file:${testImagePath2}`],
				},
				models: [
					{
						model: 'test/multi-image',
						provider: 'test-provider',
						producerId: 'TestMultiImageProducer',
					},
				],
			}),
			'utf8'
		...
		const blobRefs = inputImagesValue as Array<{
			hash: string;
			size: number;
			mimeType: string;
		}>;
