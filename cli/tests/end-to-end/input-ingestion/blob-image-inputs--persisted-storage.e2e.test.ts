...
describe('end-to-end: image-to-video with blob input', () => {
	...
	it('persists image blob to storage and creates valid plan', async () => {
				...
				models: [
					{
						...
						provider: 'fal-ai',
						producerId: 'ImageToVideoTestProducer',
					},
				],
			}),
			'utf8'
		...
		const blobRef = inputImageValue as {
			hash: string;
			size: number;
			mimeType: string;
		};
	...
	it('handles multiple image inputs correctly', async () => {
				...
				models: [
					{
						...
						producerId: 'DualImageTestProducer',
					},
				],
			}),
			'utf8'
		...
		const blobRef1 = inputImage1Value as {
			hash: string;
			size: number;
			mimeType: string;
		};
		const blobRef2 = inputImage2Value as {
			hash: string;
			size: number;
			mimeType: string;
		};
		...
		const expectedBlobPath1 = resolve(
			blobsDir,
			prefix1,
			`${blobRef1.hash}.png`
		);
		...
		const expectedBlobPath2 = resolve(
			blobsDir,
			prefix2,
			`${blobRef2.hash}.jpg`
		);
