...
describe('end-to-end: TimelineComposer with conditional segments', () => {
	...
	it('generates correct timeline with sparse fan-in data from skipped segments', async () => {
		...
		const produce: ProduceFn = vi.fn(
			async (request: ProduceRequest): Promise<ProduceResult> => {
				...
				if (producerName === 'TimelineComposer') {
					...
					// Build config matching the conditional-narration-routing input template
		...
		expect(audioClip1.properties.assetId).toContain(
		...
		expect(audioClip2.properties.assetId).toContain(
		...
		expect(videoClip1.properties.assetId).toContain(
		...
		expect(
			imageTrack!.clips.every((c) => c.startTime < 10 || c.startTime >= 20)
		).toBe(true);
