...
import { createLoggerRecorder, setupTempCliConfig } from '../helpers.js';
import {
	CATALOG_MODELS_ROOT,
...
describe('end-to-end: TimelineComposer with Transcription track', () => {
	...
	it('generates correct Transcription track alongside Audio, Video, and Image tracks', async () => {
		...
		const inputsPath = resolve(
			...
			'conditional-logic',
			'conditional-narration-routing',
			'input-template-transcription-track.yaml'
		);
		...
		const produce: ProduceFn = vi.fn(
			async (request: ProduceRequest): Promise<ProduceResult> => {
				...
				throw new Error(`Unexpected producer in test harness: ${producerName}`);
			}
		);
		...
		expect(tClip0.properties.assetId).toContain(
			'AudioProducer.GeneratedAudio[1]'
		);
		...
		expect(tClip1.properties.assetId).toContain(
			'AudioProducer.GeneratedAudio[2]'
		);
		...
		expect(tClip2.properties.assetId).toContain(
			'AudioProducer.GeneratedAudio[3]'
		);

		// No clip at startTime=0 (segment 0 has no audio)
		expect(transcriptionTrack!.clips.every((c) => c.startTime >= 10)).toBe(
			true
		);
		...
		expect(videoTrack!.clips[0].properties.assetId).toContain(
			'VideoProducer.GeneratedVideo[1]'
		);

		expect(videoTrack!.clips[1].startTime).toBe(30);
		expect(videoTrack!.clips[1].duration).toBe(10);
		expect(videoTrack!.clips[1].properties.assetId).toContain(
			'VideoProducer.GeneratedVideo[3]'
		);
		...
		expect(
			videoTrack!.clips.every((c) => c.startTime === 10 || c.startTime === 30)
		).toBe(true);

		// No image clips in segment 1 or 3 ranges
		expect(
			imageTrack!.clips.every((c) => c.startTime === 0 || c.startTime === 20)
		).toBe(true);
