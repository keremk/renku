 ...
 *
 * Uses the conditional-narration-routing blueprint which has conditions on:
...
describe('end-to-end: condition-referenced artifacts in producer graph', () => {
	...
	it('generates boolean values as proper booleans in dry-run simulation', async () => {
		...
		expect(plan.layers[0]?.some((j: any) => j.producer === 'DocProducer')).toBe(
			true
		);
	...
	it('condition evaluator correctly matches decomposed artifact values', async () => {
		...
		for (let i = 0; i < 3; i++) {
			const audioJob = audioJobs.find(
				(j: any) => j.jobId === `Producer:AudioProducer[${i}]`
			);
