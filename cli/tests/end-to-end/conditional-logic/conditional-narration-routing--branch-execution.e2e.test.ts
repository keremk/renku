...
describe('end-to-end: conditional edge execution', () => {
	...
	it('executes only matching conditional branches based on NarrationType', async () => {
		const blueprintPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'conditional-logic', 'conditional-narration-routing',
			'conditional-narration-routing.yaml'
		);
		const inputsPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'conditional-logic', 'conditional-narration-routing',
			'input-template.yaml'
		);
		...
		// - 1 TimelineComposer
		const docJobs = initialJobs.filter(
			(j: any) => j.producer === 'DocProducer'
		);
		const imageJobs = initialJobs.filter(
			(j: any) => j.producer === 'ImageProducer'
		);
		const audioJobs = initialJobs.filter(
			(j: any) => j.producer === 'AudioProducer'
		);
		const videoJobs = initialJobs.filter(
			(j: any) => j.producer === 'VideoProducer'
		);
		const timelineJobs = initialJobs.filter(
			(j: any) => j.producer === 'TimelineComposer'
		);
		...
		const produce: ProduceFn = vi.fn(
			async (request: ProduceRequest): Promise<ProduceResult> => {
				...
				if (
					!['ImageProducer', 'AudioProducer', 'VideoProducer'].includes(
						request.job.producer
					)
				) {
					throw new Error(
						`Unexpected producer in test harness: ${request.job.producer}`
					);
				}

				// Supported media producers return stub data for their artifacts
		...
		expect(
			succeededJobs.filter((j) => j.producer === 'DocProducer')
		).toHaveLength(1);

		// TimelineComposer always runs
		expect(
			succeededJobs.filter((j) => j.producer === 'TimelineComposer')
		).toHaveLength(1);

		// ImageProducer: [0][0], [0][1], [2][0], [2][1] succeed (ImageNarration)
		// ImageProducer: [1][0], [1][1] skipped (TalkingHead, not ImageNarration)
		expect(
			succeededJobs.filter((j) => j.producer === 'ImageProducer')
		).toHaveLength(4);
		expect(
			skippedJobs.filter((j) => j.producer === 'ImageProducer')
		).toHaveLength(2);
		...
		expect(
			succeededJobs.filter((j) => j.producer === 'AudioProducer')
		).toHaveLength(2);
		expect(
			skippedJobs.filter((j) => j.producer === 'AudioProducer')
		).toHaveLength(1);

		// VideoProducer: [1] succeeds (TalkingHead)
		// VideoProducer: [0], [2] skipped (ImageNarration, not TalkingHead)
		expect(
			succeededJobs.filter((j) => j.producer === 'VideoProducer')
		).toHaveLength(1);
		expect(
			skippedJobs.filter((j) => j.producer === 'VideoProducer')
		).toHaveLength(2);
		...
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[0][0]')
			)
		).toBe(true);
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[0][1]')
			)
		).toBe(true);
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[2][0]')
			)
		).toBe(true);
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[2][1]')
			)
		).toBe(true);
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[1][0]')
			)
		).toBe(false); // SKIPPED
		expect(
			artifactIds.some((id) =>
				id.includes('ImageProducer.GeneratedImage[1][1]')
			)
		).toBe(false); // SKIPPED

		// GeneratedAudio: [1], [2] EXIST; [0] DOES NOT EXIST
		expect(
			artifactIds.some((id) => id.includes('AudioProducer.GeneratedAudio[1]'))
		).toBe(true);
		expect(
			artifactIds.some((id) => id.includes('AudioProducer.GeneratedAudio[2]'))
		).toBe(true);
		expect(
			artifactIds.some((id) => id.includes('AudioProducer.GeneratedAudio[0]'))
		).toBe(false); // SKIPPED

		// GeneratedVideo: [1] EXISTS; [0], [2] DO NOT EXIST
		expect(
			artifactIds.some((id) => id.includes('VideoProducer.GeneratedVideo[1]'))
		).toBe(true);
		expect(
			artifactIds.some((id) => id.includes('VideoProducer.GeneratedVideo[0]'))
		).toBe(false); // SKIPPED
		expect(
			artifactIds.some((id) => id.includes('VideoProducer.GeneratedVideo[2]'))
		).toBe(false); // SKIPPED
