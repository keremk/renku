...
describe('end-to-end: image-to-video dry runs', () => {
  ...
  it('wires sliding start/end images into ImageToVideoProducer', async () => {
    const blueprintPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'pipeline-orchestration', 'video-continuity-sliding-images', 'video-continuity-sliding-images.yaml');
    const inputsPath = resolve(CLI_FIXTURES_INPUTS, 'video-continuity-sliding-images--default.inputs.yaml');
