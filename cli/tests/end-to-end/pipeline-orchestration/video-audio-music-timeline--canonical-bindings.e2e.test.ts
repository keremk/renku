...
describe('end-to-end: video-audio-music dry runs', () => {
	...
	it('runs query and edit dry-runs with canonical bindings and artifacts', async () => {
		...
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'video-audio-music-timeline--default.inputs.yaml'
		);
		...
		expect(audioJob0?.context?.inputBindings?.Text).toMatch(
			/^Artifact:ScriptProducer\.NarrationScript\[0]/
		);
		expect(
			audioJob0?.inputs.some((id: string) =>
				id.startsWith('Artifact:ScriptProducer.NarrationScript')
			)
		).toBe(true);

		const videoPromptJobs = plan.layers
			.flat()
			.filter((job: any) => job.producer === 'VideoPromptProducer');
		expect(videoPromptJobs).toHaveLength(2);
		const videoPromptJob0 = videoPromptJobs[0];
		expect(
			videoPromptJob0?.inputs.some((id: string) =>
				id.startsWith('Artifact:ScriptProducer.NarrationScript')
			)
		).toBe(true);
		...
		const videoJobs = plan.layers
			.flat()
			.filter((job: any) => job.producer === 'VideoProducer');
		...
		for (const job of videoJobs) {
			...
			expect(
				job.inputs.some((id: string) =>
					id.startsWith('Artifact:VideoPromptProducer.VideoPrompt')
				)
			).toBe(true);
		}
		...
		expect(
			musicPromptJob?.context?.fanIn?.[
				'Input:MusicPromptProducer.NarrationScript'
			]?.members?.length
		).toBeGreaterThan(0);
		expect(musicPromptJob?.context?.inputBindings?.NarrationScript).toBe(
			'Input:MusicPromptProducer.NarrationScript'
		);

		const musicJob = findJob(plan, 'MusicProducer');
		expect(musicJob?.context?.inputBindings?.Prompt).toBe(
			'Artifact:MusicPromptProducer.MusicPrompt'
		);
		...
		expect(
			timelineJob.context?.fanIn?.['Input:TimelineComposer.Music']?.members
				?.length
		).toBe(1);

		// Dry-run plans are transient now, so the debug plan is available through
		// result.planPath instead of being recorded under the movie workspace.
