...
describe('end-to-end: derived video artifacts', () => {
	...
	it('plans derived artifacts (FirstFrame, LastFrame, AudioTrack) and wires LastFrame to downstream producer', async () => {
		const blueprintPath = resolve(
			...
			'derived-video-frames-audio',
			'derived-video-frames-audio.yaml'
		);
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'derived-video-frames-audio--default.inputs.yaml'
		);
		...
		expect(firstVideoJob.produces).toContain(
			'Artifact:FirstVideoProducer.GeneratedVideo'
		);
		expect(firstVideoJob.produces).toContain(
		...
		expect(secondVideoJob.inputs).toContain(
			'Artifact:FirstVideoProducer.LastFrame'
		);
	...
	it('correctly tracks all derived artifacts in the plan', async () => {
		...
		expect(allProducedArtifacts).toContain(
			'Artifact:SecondVideoProducer.GeneratedVideo'
		);
	});
});

describe('end-to-end: real ffmpeg extraction', () => {
	const VIDEO_FIXTURE_PATH = resolve(
		CLI_FIXTURES_MEDIA,
		'derived-video-frames-audio--source.mp4'
	);
