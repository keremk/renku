...
describe('end-to-end: derived panel images', () => {
	...
	it('plans panel artifacts (PanelImages[0-8]) and includes them in produces', async () => {
		const blueprintPath = resolve(
			...
			'derived-storyboard-panels',
			'derived-storyboard-panels.yaml'
		);
		const inputsPath = resolve(
			CLI_FIXTURES_INPUTS,
			'derived-storyboard-panels--3x3.inputs.yaml'
		);
		...
		if (result.build?.status !== 'succeeded') {
			throw new Error(
				`dryRun failed: ${JSON.stringify(result.build, null, 2)}`
			);
		}
		...
		expect(imageJob.produces).toContain(
			'Artifact:ImageGenerator.GeneratedImage'
		);
		for (let i = 0; i < 9; i++) {
			expect(imageJob.produces).toContain(
				`Artifact:ImageGenerator.PanelImages[${i}]`
			);
		}
	...
	it('correctly tracks all panel artifacts in the plan', async () => {
		...
		for (let i = 0; i < 9; i++) {
			expect(allProducedArtifacts).toContain(
				`Artifact:ImageGenerator.PanelImages[${i}]`
			);
		}
...
describe('end-to-end: real ffmpeg panel extraction', () => {
	const GRID_IMAGE_FIXTURE_PATH = resolve(
		SHARED_TEST_MEDIA_ROOT,
		'grid-image-fixture.jpeg'
	);
	...
	it('extracts all 9 panels from real 3x3 grid image', async () => {
		...
		const produces = [
			...
			...Array.from(
				{ length: 9 },
				(_, i) => `Artifact:TestProducer.PanelImages[${i}]`
			),
		];
	...
	it('extracts panels with correct grid positions for 3x3', async () => {
		...
		for (let i = 0; i < 9; i++) {
			expect(result.panels[i].diagnostics?.gridPosition).toEqual(
				expectedPositions[i]
			);
		}
	});
	...
	it('extracts panels with correct crop dimensions (640x360 for 1920x1080 3x3)', async () => {
		...
		expect(panel0?.diagnostics?.crop).toEqual({
			x: 0,
			y: 0,
			width: 640,
			height: 360,
		...
		expect(panel4?.diagnostics?.crop).toEqual({
			x: 640,
			y: 360,
			width: 640,
			height: 360,
		...
		expect(panel8?.diagnostics?.crop).toEqual({
			x: 1280,
			y: 720,
			width: 640,
			height: 360,
	...
	it('produces different content for different panels', async () => {
		...
		const panel0Data = result.panels.find(
			(p) => p.diagnostics?.panelIndex === 0
		)?.blob?.data;
		const panel4Data = result.panels.find(
			(p) => p.diagnostics?.panelIndex === 4
		)?.blob?.data;
		const panel8Data = result.panels.find(
			(p) => p.diagnostics?.panelIndex === 8
		)?.blob?.data;
		...
		expect(Buffer.compare(panel0Data as Buffer, panel4Data as Buffer)).not.toBe(
			0
		);
		expect(Buffer.compare(panel0Data as Buffer, panel8Data as Buffer)).not.toBe(
			0
		);
		expect(Buffer.compare(panel4Data as Buffer, panel8Data as Buffer)).not.toBe(
			0
		);
	});
	...
	it('handles partial panel requests (only some panels)', async () => {
		...
		const extractedIndices = result.panels
			.map((p) => p.diagnostics?.panelIndex)
			.sort();
	...
	it('handles 2x2 grid extraction correctly', async () => {
		...
		expect(panel3?.diagnostics?.crop).toEqual({
			x: 960,
			y: 540,
			width: 960,
			height: 540,
		...
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 1)?.diagnostics
				?.gridPosition
		).toEqual({ row: 0, col: 1 });
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 2)?.diagnostics
				?.gridPosition
		).toEqual({ row: 1, col: 0 });
	...
	it('handles non-square grid (2x3) extraction correctly', async () => {
		...
		const panel0 = result.panels.find((p) => p.diagnostics?.panelIndex === 0);
		const crop0 = panel0?.diagnostics?.crop as
		...
		// [4] [5]
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 0)?.diagnostics
				?.gridPosition
		).toEqual({ row: 0, col: 0 });
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 1)?.diagnostics
		...
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 3)?.diagnostics
				?.gridPosition
		).toEqual({ row: 1, col: 1 });
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 4)?.diagnostics
				?.gridPosition
		).toEqual({ row: 2, col: 0 });
		expect(
			result.panels.find((p) => p.diagnostics?.panelIndex === 5)?.diagnostics
				?.gridPosition
		).toEqual({ row: 2, col: 1 });
	});
});
