...
/**
 * Load the transcription-karaoke--stt-output.fixture.json fixture and parse it as STT output.
 */
async function loadTranscriptionFixture(): Promise<STTOutput> {
  const content = await readFile(resolve(CLI_FIXTURES_SCHEMAS, 'transcription-karaoke--stt-output.fixture.json'), 'utf8');
...
describe('end-to-end: transcription producer plan validation', () => {
  ...
  it('includes TranscriptionProducer in plan with correct bindings', async () => {
    const blueprintPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'pipeline-orchestration', 'video-audio-music-timeline', 'video-audio-music-timeline.yaml');
    const inputsPath = resolve(CLI_FIXTURES_INPUTS, 'video-audio-music-timeline--transcription-pipeline.inputs.yaml');
