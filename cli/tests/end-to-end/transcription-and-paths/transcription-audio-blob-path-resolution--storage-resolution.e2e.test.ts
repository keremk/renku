...
async function runTranscriptionPathScenario(
  ...
  const blueprintPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'transcription-and-paths', 'transcription-audio-blob-path-resolution',
    'transcription-audio-blob-path-resolution.yaml',
  );
  const inputsPath = resolve(CLI_FIXTURES_BLUEPRINTS, 'transcription-and-paths', 'transcription-audio-blob-path-resolution',
    'input-template.yaml',
  );
