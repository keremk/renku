...
models:
  - provider: openai
    model: gpt-4o
    promptFile: ./documentary-prompt.toml
    outputSchema: ./documentary-prompt-output.json
    config:
      text_format: json_schema
```
...
The `outputSchema` from the producer's model configuration is automatically associated with `type: json` artifacts. The schema defines the structure of the JSON output:
