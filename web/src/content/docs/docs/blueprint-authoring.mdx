...
Use `groupBy: singleton` when several upstream values should become one ordered collection for a single downstream input. Add `orderBy: <dimension>` when values come from a loop and the provider-facing order matters.
...
This shape keeps the selected character references in one media list while ordering them by the declared `historicalcharacter` dimension. For Seedance prompts, that means `@Image1` and `@Image2` can refer to the first selected character's portrait and sheet, while `@Image3` and `@Image4` refer to the next selected character.

Use `groupBy: <dimension>` instead when each loop item should become its own fan-in group. For example, a timeline composer usually groups by `segment` and orders by `clip`.

Wrapper producers can also forward a fan-in collection into a nested producer:
...
The leaf producer can then use `flattenFanIn: true` in its SDK mapping to send the grouped values as a flat provider array.
...
Fan-in inputs can be optional. If an optional fan-in input has no inbound binding, the planner and viewer treat it as absent rather than inventing an empty or default value.
