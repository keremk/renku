...
The nested producer then receives the same collection. In the leaf producer's provider mapping, `flattenFanIn: true` converts Renku's grouped fan-in value into the plain array expected by the provider API:
...
In plain English: Renku tracks fan-in as grouped graph data; the provider API wants `image_urls: [...]`; `flattenFanIn` is the last-mile conversion between those two shapes.
