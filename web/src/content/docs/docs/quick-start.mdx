...
description: Set up the Renku CLI workflow and generate your first AI video
---
...
For installation instructions, see the [Install Page](/install).

## Setup

<Aside>
  If you already completed onboarding through the UI using `renku launch`, you
  can skip this setup section and jump to [Run Your First
  Blueprint](#run-your-first-blueprint).
</Aside>
...
<Aside>
  Builds and artifacts are created in your **current working directory** when
  running `renku generate`, not in the workspace root. This supports
...
The copied blueprint uses the current authored schema: `inputs:`, `outputs:`, and `imports:`.
...
This opens the Renku app, starts the singleton local viewer server if needed, and opens your browser to preview the generated content.
...
│       ├── events/              # Input/artifact history logs (JSONL)
...
The `artifacts/` directory contains human-readable symlinks to your generated content. Internally, Renku reconstructs the current `BuildState` from `events/` and `runs/` rather than reading a stored manifest file. Use `renku list` to see all builds in the current project.
...
Once installed, you can use the `renku-design-blueprint` skill to have Claude help you design and create blueprints through natural conversation:
...
- Understanding whether you want an asset pipeline, a compositing pipeline, or a fully rendered video pipeline
