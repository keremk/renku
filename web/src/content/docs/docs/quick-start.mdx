...
| --------------------- | --------------------------- | ----------------------------------------- |
| macOS (Apple Silicon) | Desktop app or CLI          | Native app installer is available         |
| macOS (Intel)         | CLI + browser viewer        | Native app installer is not available yet |
| Linux                 | CLI + browser viewer        | Fully supported through npm installation  |
| Windows               | CLI + browser viewer (WSL2) | Use a Linux distro in WSL2                |

For platform-specific install options, see the [Install Page](/install).
...
- **FFmpeg & FFprobe** - For native video export and other local video processing. Download from [ffmpeg.org](https://ffmpeg.org/download.html)

## Installation

Install the Renku CLI globally (Linux, macOS, and Windows via WSL):
...
For the npm-installed CLI and the desktop/viewer flow, Renku reads API keys from `~/.config/renku/.env`.
