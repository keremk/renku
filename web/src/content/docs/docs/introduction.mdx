...
┌─────────────────────────────────────────────────────────────────────┐
│                           Blueprint YAML                            │
│  • Defines workflow: inputs, outputs, imports, connections          │
└─────────────────────────────────────────────────────────────────────┘
                                   │
                                   ▼
┌─────────────────────────────────────────────────────────────────────┐
...
- What output connectors the blueprint exposes
- Which child blueprints it imports and how to connect them
...
Any runtime product produced by a producer job:
...
Artifacts are tracked under canonical `Artifact:...` IDs in the runtime event log and in reconstructed `BuildState` snapshots. They can be scalar (single value) or arrays (one per segment).
...
Data flow edges that wire outputs to inputs. Connections define how data flows from one producer to another:

- Blueprint input → Producer input
- Producer artifact → Another producer's input
- Producer artifact → Blueprint output connector
...
- Blueprint catalog
- Build outputs (artifacts, event logs, run records)
- Configuration

## What's Next?

Ready to try Renku? Start with the [Install Page](/install), then continue to the [CLI Quick Start](/docs/quick-start) to generate your first video.
