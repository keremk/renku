...
Flattens a fan-in value into the provider array field. Use this when a producer input is declared with `fanIn: true` and the provider expects a plain list such as `image_urls`, `video_urls`, or `audio_urls`.
...
Fan-in values preserve the graph's group and order metadata before flattening. If no value is connected to an optional fan-in input, the field is not sent to the provider.
