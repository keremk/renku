...
### 4b. flattenFanIn - Grouped Fan-In to Array
...
`flattenFanIn: true` is the producer mapping step that turns Renku's internal fan-in value into the provider's array field.
...
Provider payload after flattenFanIn:
  image_urls: [portraitBlob, sheetBlob]
```

The provider file resolver then sees that `image_urls` is an array of URI fields in the model schema. Any local blob values in that array are uploaded through the provider adapter and replaced with uploaded URLs before the SDK call.

Use `flattenFanIn` when all of these are true:

- the producer input is declared with `fanIn: true`,
- the mapping target is a provider array field,
- the provider model schema expects URLs or scalar values, not Renku fan-in metadata.

If no value is connected to an optional fan-in input, the field is not sent to the provider.

Seedance reference prompts must refer to supplied media with one-based labels by media type: `@Image1`, `@Image2`, `@Video1`, `@Audio1`, and so on. Only mention labels for media that the graph actually supplies.
...
5. **flattenFanIn** - Flatten grouped fan-in values
6. **invert** - Flip boolean
7. **intToString** - Convert to string
8. **intToSecondsString** - Convert to string with "s"
9. **durationToFrames** - Multiply by fps
10. **transform** - Value lookup
11. **expand** - Spread object into payload
