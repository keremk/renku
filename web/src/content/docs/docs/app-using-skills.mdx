...
2. Run the blueprint skill (`/renku-plugin:renku-design-blueprint` in Claude Code, or `$renku-design-blueprint` in Codex).

3. Describe the workflow you want, including whether it should stop at reusable assets and metadata or continue through composition and final export.

4. Review the generated blueprint in your workspace, then run it with Renku.

</Steps>

Skills are meant to teach the agent the same safety rules Renku enforces. In particular, top-level blueprint outputs are publication endpoints, not internal routing nodes. For the human explanation of that rule, preflight, dry-run coverage, and stage-by-stage validation, see [Blueprint Validation and Preflight](/docs/blueprint-validation-and-preflight).
