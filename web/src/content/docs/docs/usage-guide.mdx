...
| Multiple specific segments need work | `--aid=X --aid=Y` |
...
```bash
renku generate --last --aid="AudioProducer.GeneratedAudio[2]" --inputs=./inputs.yaml
```
...
6. **Use `--aid` for surgical fixes** - If specific segments need work, regenerate just those artifacts instead of the whole layer (e.g., `--aid=Audio[0] --aid=Audio[2]`)
