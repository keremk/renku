...
Use `--pid` when you want to scope planning by producer family and optional first-dimension count.
...
- When `--pid` is present, producer scope drives planning (layer cap via `--up-to-layer` is ignored)
...
- If the same artifact is both pinned and explicitly regenerated, regenerate wins for that artifact
