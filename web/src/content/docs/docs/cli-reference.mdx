...
renku generate --movie-id=<id> --regen="Artifact:AudioProducer.GeneratedAudio[0]" --regen="Artifact:AudioProducer.GeneratedAudio[2]" --inputs=<path> [--up-to-layer=<n>]
renku generate --movie-id=<id> --pid="Producer:AudioProducer:1" --inputs=<path>
renku generate --movie-id=<id> --pin="Artifact:ScriptProducer.NarrationScript[0]" --pin="Producer:ImageProducer" --inputs=<path> [options]
```
...
| `--preflight-only`                     | Validate, load inputs, resolve model/producer metadata, build the plan, and exit before dry-run or live execution                                                             |
| `--dry-run`                            | Execute without calling providers (mock run)                                                                                                                                   |
| `--dry-run-profile`, `--profile`       | Use a pre-generated dry-run profile for simulation coverage (requires `--dry-run`)                                                                                             |
| `--non-interactive`                    | Skip confirmation prompts                                                                                                                                                      |
| `--explain`                            | Print plan explanation and cost summary, then exit without execution                                                                                                           |
...
| `--pid`, `--producer-id`               | Producer scheduling scope. Format: `Producer:Alias:<count>` (count required). Repeatable. Includes required upstream dependencies.                                            |
...
2. Runs preflight planning checks
3. Generates a new movie ID
4. Creates `builds/movie-{id}/` directory
...
2. Rebuilds the current `BuildState` from event logs and run records
3. Computes the baseline plan from graph dependency analysis + dirty detection
4. Applies explicit overrides in this order:
   - Add explicit regenerate targets from `--regen`
   - Apply pin exclusions from `--pin` (direct `pin` vs `regen` overlap on the same target is a hard error)
   - Apply scope constraints from both `--up-to-layer` and `--pid` together
   - Ignore out-of-scope controls with planning warnings
...
If the blueprint ends in `VideoExporter`, `renku generate` also materializes the final published render and prints the published output path in the command summary.
...
# Regenerate one artifact and its downstream dependencies
renku generate --movie-id=<id> --regen="Artifact:AudioProducer.GeneratedAudio[0]" --inputs=./inputs.yaml

# Regenerate using producer-level target (producer + downstream)
...
# Scope to a producer family (first segment only)
...
**Note:** Non-canonical `--regen` values are rejected; use canonical `Artifact:...` or `Producer:...` IDs.  
**Note:** `--pid` values must include an explicit count (`Producer:Alias:<count>`).  
**Note:** `--pid` and `--up-to-layer` are combined; both constraints apply.  
**Note:** If the same target appears in both `--pin` and `--regen`, the command fails with a conflict error.
...
- Shows which builds have artifacts (completed runs) vs. builds with no materialized outputs
...
This command performs static validation, prepared graph checks, and viewer projection checks. For plan-specific checks with real inputs, use `renku generate --preflight-only`. For simulated execution coverage, use `renku generate --dry-run`.

For the conceptual difference between validation, preflight, and dry-run, see [Blueprint Validation and Preflight](/docs/blueprint-validation-and-preflight).
...
- Connection validity
- Loop definitions
- Viewer graph projection
- Published-output endpoint rules
...
| Option     | Required | Description                                                          |
| ---------- | -------- | -------------------------------------------------------------------- |
| `--output` | No       | Output profile path (defaults to `<blueprint>.dry-run-profile.yaml`) |
...
| `blueprint` | string | No       | Blueprint path for sanity-checking profile usage            |
| `inputs`    | string | No       | Inputs path for sanity-checking profile usage               |
| `generator` | object | No       | Metadata (`cases`, `seed`) for generated profiles           |
| `cases`     | array  | No       | Explicit simulation cases (generated profiles include this) |

**Case object:**

| Field            | Type   | Required | Description                         |
| ---------------- | ------ | -------- | ----------------------------------- |
| `id`             | string | Yes      | Case label shown in dry-run summary |
| `conditionHints` | object | No       | Per-case simulation overrides       |

**`conditionHints` object:**

| Field           | Type  | Required | Description                                          |
| --------------- | ----- | -------- | ---------------------------------------------------- |
| `mode`          | enum  | Yes      | One of `first-value`, `alternating`, `comprehensive` |
| `varyingFields` | array | Yes      | Condition-driven fields to vary                      |

**`varyingFields[]` object:**

| Field        | Type   | Required | Description                                                                                                             |
| ------------ | ------ | -------- | ----------------------------------------------------------------------------------------------------------------------- |
| `artifactId` | string | Yes      | Canonical field artifact ID (for example `Artifact:StoryProducer.Storyboard.Scenes[scene].CharacterPresent[character]`) |
| `values`     | array  | Yes      | Candidate values used by simulation                                                                                     |
| `dimension`  | string | No       | Preferred varying dimension when relevant                                                                               |
...
Open the Renku app, optionally deep-linking to a blueprint.

**Usage:**

```bash
renku launch [blueprint-name]
```
...
| Argument | Required | Description                                      |
| -------- | -------- | ------------------------------------------------ |
| `[blueprint-name]` | No       | Blueprint folder name (for example `style-cartoon`). |
...
| `--port`       | Override viewer port (optional) |

**Behavior:**

- Runs a singleton background server (does not start a second instance on repeated launches with the same host/port)
- If a managed server is running on a different host/port, Renku shuts it down before launching the requested one
- Uses deterministic port selection (no random fallback)
- Opens `/blueprints` in your browser
- If `[blueprint-name]` is provided, opens `/blueprints?bp=<blueprint-name>`
...
Stop the singleton background viewer server.
...
│       │   └── artifacts.log # Artifact events (JSONL)
│       ├── runs/
│       │   ├── {rev}-run.json
│       │   ├── {rev}-plan.json
│       │   └── {rev}-inputs.yaml
...
Renku does not persist a separate manifest snapshot in this layout. The current `BuildState` is rebuilt from `events/` and `runs/` whenever the CLI or viewer needs the latest state.
