...
| `--pid`, `--producer-id`               | Producer scheduling scope. Format: `Producer:Alias` or `Producer:Alias:<count>`. Repeatable. Includes required upstream dependencies.                                         |
...
4. Applies explicit overrides in this order:
   ...
   - Apply pin exclusions from `--pin` (except direct `pin` vs `regen` conflicts, where regenerate wins)
   - Apply layer limit from `--up-to-layer` (ignored when `--pid` scope is present)
...
**Note:** `--pid` and `--up-to-layer` are not combined; when `--pid` is present, producer scope drives planning.  
**Note:** If the same artifact is both pinned and explicitly regenerated in one command, regenerate wins for that artifact.
