...
| `--artifact-id`, `--artifact`, `--aid` | Regenerate specific artifact(s) and their downstream dependencies (requires `--last` or `--movie-id`). Requires canonical `Artifact:...` IDs. Can be specified multiple times. |
