...
renku generate --last --pin=<canonical-id> --pin=<canonical-id> --inputs=<path> [options]
```
...
| `--artifact-id`, `--artifact`, `--aid` | Regenerate specific artifact(s) and their downstream dependencies (requires `--last` or `--movie-id`). Prefer canonical IDs (`Artifact:...`). Can be specified multiple times. |
...
**Note:** Short non-canonical `--artifact-id/--aid` values are deprecated; prefer canonical `Artifact:...` IDs.
