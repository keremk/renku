...
export type DropzoneMediaType = MediaInputType | 'file';
...
const FILE_TYPE_DESCRIPTION = 'Any file up to 100MB';

interface DropzoneAreaProps {
  /** Media type to accept */
  mediaType: DropzoneMediaType;
...
export function DropzoneArea({
  ...
  const { getRootProps, getInputProps, isDragActive, isDragReject } =
    useDropzone({
      onDrop,
      accept: mediaType === 'file' ? undefined : ACCEPT_TYPES[mediaType],
      multiple,
      maxSize: MAX_FILE_SIZE,
    });

  const typeDescription =
    mediaType === 'file' ? FILE_TYPE_DESCRIPTION : TYPE_DESCRIPTIONS[mediaType];
