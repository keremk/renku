...
interface FalRunArgs {
  fal: { run: (id: string, opts: { input: Record<string, unknown> }) => Promise<unknown> };
...
export async function runFalWithRetries(args: FalRunArgs): Promise<unknown> {
  const {
    fal,
    modelIdentifier,
    input,
  ...
  while (attempt < maxAttempts) {
    attempt += 1;
    try {
      return await fal.run(modelIdentifier, { input });
    } catch (error: unknown) {
      lastError = error;
      ...
      if (!isThrottled) {
        ...
        throw new Error(String((error as any)?.message ?? error ?? 'Fal.ai prediction failed.'));
      }

      if (shouldRetry) {
        sawThrottle = true;
        logger?.info?.(
          `Fal.ai provider will retry after ${retryMs}ms for the job ${jobId}. Attempt #${attempt}`,
        );
        logger?.debug?.('providers.fal.retry', {
          producer: jobId,
          model,
          plannerContext,
          status,
        ...
        logger?.info?.(`Fal.ai provider for job ${jobId} using model ${model} waited for ${waitedMs}ms`);
        logger?.debug?.('providers.fal.retry.waited', {
          producer: jobId,
          model,
          attempt,
          waitedMs: waitedMs,
  ...
  const message = 'Fal.ai rate limit hit (429); retries exhausted. Lower concurrency, wait, or add credit.';
  if (sawThrottle) {
  ...
  throw new Error(String((lastError as any)?.message ?? lastError ?? 'Fal.ai prediction failed.'));
}
