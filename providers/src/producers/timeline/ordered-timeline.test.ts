...
function makeRequest(
  options: { omitAudio?: boolean; audioDurations?: number[] } = {}
): ProviderJobContext {
  const imageGroups = [
  ...
  const audioGroups = options.omitAudio
    ? []
    : [['Artifact:Audio[0]'], ['Artifact:Audio[1]']];
  ...
  const resolvedInputs: Record<string, unknown> = {
    ...
    'TimelineComposer.AudioSegments': {
      groupBy: 'segment',
      groups: audioGroups,
    },
  ...
  if (options.omitAudio) {
    ...
    audioGroups.forEach((group, index) => {
      ...
      const payload = createAssetPayload(
        audioDurations[index] ?? audioDurations[0] ?? 1
      );
...
describe('TimelineProducer', () => {
  it('builds a timeline document with aligned tracks', async () => {
    ...
    expect(timeline.assetFolder?.rootPath).toBe(
      '/tmp/timeline-root/builds/movie-abc'
    );
    ...
    expect(imageTrack?.clips[0]?.properties.effects?.[0]?.assetId).toBe(
      'Artifact:Image[0][0]'
    );
  });
  ...
  it('throws when tracks are not provided', async () => {
    ...
    const config = request.context.providerConfig as {
      config: { timeline: Record<string, unknown> };
    };
  ...
  it('emits video clips with original durations and always uses stretch strategy', async () => {
    ...
    const timeline = JSON.parse(
      ...
      tracks: Array<{
        kind: string;
        clips: Array<{ duration: number; properties: Record<string, unknown> }>;
      }>;
    };
  ...
  it('throws when master track is not included in configured tracks', async () => {
    ...
    const request = makeRequest();
    const config = request.context.providerConfig as {
      config: { timeline: { tracks?: string[]; masterTracks?: string[] } };
    };
    config.config.timeline.tracks = ['Audio'];
  ...
  it('builds a Transcription track with correct clip timing', async () => {
    ...
    resolvedInputs['Input:TimelineComposer.TranscriptionAudio'] =
      transcriptionFanIn;
    ...
    const transcriptionTrack = timeline.tracks.find(
      (track) => track.kind === 'Transcription'
    );
    ...
    expect(transcriptionTrack?.clips[0]?.properties.assetId).toBe(
      'Artifact:TranscriptionAudio[0]'
    );
    expect(transcriptionTrack?.clips[1]?.startTime).toBeCloseTo(12);
    expect(transcriptionTrack?.clips[1]?.duration).toBeCloseTo(8);
    expect(transcriptionTrack?.clips[1]?.properties.assetId).toBe(
      'Artifact:TranscriptionAudio[1]'
    );
  });
  ...
  it('builds Transcription track from shorthand transcriptionClip config', async () => {
    ...
    const timeline = JSON.parse(
      ...
      tracks: Array<{
        kind: string;
        clips: Array<{ properties: Record<string, unknown> }>;
      }>;
    };
    const transcriptionTrack = timeline.tracks.find(
      (track) => track.kind === 'Transcription'
  ...
  it('builds Transcription track with mixed audio sources from different producers', async () => {
    ...
    resolvedInputs['Artifact:AudioProducer.GeneratedAudio[0]'] =
      createAssetPayload(10);
    resolvedInputs['Artifact:AudioProducer.GeneratedAudio[1]'] =
      createAssetPayload(8);
    resolvedInputs['Artifact:AudioProducer.GeneratedAudio[2]'] =
      createAssetPayload(7);
    resolvedInputs['Artifact:AudioProducer.GeneratedAudio[3]'] =
      createAssetPayload(10);
    ...
    resolvedInputs['Input:TimelineComposer.ImageSegments'] = {
      groupBy: 'segment',
      orderBy: 'image',
      groups: imageGroups,
    };
    resolvedInputs['TimelineComposer.ImageSegments'] = {
      groupBy: 'segment',
      orderBy: 'image',
      groups: imageGroups,
    };
    ...
    resolvedInputs['Input:TimelineComposer.TranscriptionAudio'] = {
      groupBy: 'segment',
      groups: transcriptionGroups,
    };
    resolvedInputs['TimelineComposer.TranscriptionAudio'] = {
      groupBy: 'segment',
      groups: transcriptionGroups,
    };
    // AudioProducer assets already registered above; add lipsync asset
    resolvedInputs['Artifact:LipsyncVideo.AudioTrack[1]'] =
      createAssetPayload(8);
    ...
    expect(transcriptionTrack?.clips[1]?.duration).toBeCloseTo(8);
    expect(transcriptionTrack?.clips[1]?.properties.assetId).toBe(
      'Artifact:LipsyncVideo.AudioTrack[1]'
    );
    ...
    expect(transcriptionTrack?.clips[2]?.properties.assetId).toBe(
      'Artifact:AudioProducer.GeneratedAudio[3]'
    );
  });
  ...
  it('builds Transcription track with interleaved sparse pattern across 6 segments', async () => {
    ...
    const audioGroups = Array.from({ length: 6 }, (_, i) => [
      `Artifact:Audio[${i}]`,
    ]);
    resolvedInputs['Input:TimelineComposer.AudioSegments'] = {
      groupBy: 'segment',
    ...
    expect(transcriptionTrack?.clips[2]?.properties.assetId).toBe(
      'Artifact:TranscriptionAudio[5]'
    );
  });

  it('builds a Text track with per-snippet timing and default transition', async () => {
    const handler = createHandler();
    const request = makeRequest();
    const resolvedInputs = request.context.extras?.resolvedInputs as Record<
      string,
    ...
    config.config.timeline.clips.push({ kind: 'Text', inputs: 'TextSegments' });
    config.config.timeline.numTracks = 3;
    config.config.timeline.tracks = ['Image', 'Audio', 'Text'];
    request.inputs.push('Input:TimelineComposer.TextSegments');

    const textFanIn = {
      groupBy: 'segment',
      groups: [
        ['Artifact:TextSegments[0][0]', 'Artifact:TextSegments[0][1]'],
        ['Artifact:TextSegments[1][0]'],
      ],
    };
    resolvedInputs['Input:TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['Artifact:TextSegments[0][0]'] = 'Opening scene';
    resolvedInputs['Artifact:TextSegments[0][1]'] = 'A quiet city wakes';
    resolvedInputs['Artifact:TextSegments[1][0]'] =
    ...
    const textTrack = timeline.tracks.find((track) => track.kind === 'Text');
    expect(textTrack).toBeDefined();
    expect(textTrack?.clips).toHaveLength(2);

    const firstClipEffects = textTrack?.clips[0]?.properties.effects as Array<{
      text: string;
      transition: string;
      startTime: number;
      duration: number;
    }>;
    expect(firstClipEffects).toHaveLength(2);
    expect(firstClipEffects?.[0]?.text).toBe('Opening scene');
    expect(firstClipEffects?.[0]?.transition).toBe('fade-in-out');
    expect(firstClipEffects?.[0]?.startTime).toBe(0);
    expect(firstClipEffects?.[0]?.duration).toBeCloseTo(6);
    expect(firstClipEffects?.[1]?.text).toBe('A quiet city wakes');
    expect(firstClipEffects?.[1]?.startTime).toBeCloseTo(6);
    expect(firstClipEffects?.[1]?.duration).toBeCloseTo(6);

    const secondClipEffects = textTrack?.clips[1]?.properties.effects as Array<{
      text: string;
      transition: string;
      startTime: number;
      duration: number;
    }>;
    expect(secondClipEffects).toHaveLength(1);
    expect(secondClipEffects?.[0]?.transition).toBe('fade-in-out');
    expect(secondClipEffects?.[0]?.startTime).toBeCloseTo(12);
    expect(secondClipEffects?.[0]?.duration).toBeCloseTo(8);
  });

  it('maps collapsed text fan-in groups to segments by canonical asset index', async () => {
    const handler = createHandler();
    const request = makeRequest();
    const resolvedInputs = request.context.extras?.resolvedInputs as Record<
      string,
    ...
    const collapsedTextFanIn = {
      groupBy: 'segment',
      groups: [
        [
          'Artifact:ScriptProducer.SegmentText[0]',
          'Artifact:ScriptProducer.SegmentText[1]',
        ],
        [],
      ],
    };
    resolvedInputs['Input:TimelineComposer.TextSegments'] = collapsedTextFanIn;
    resolvedInputs['TimelineComposer.TextSegments'] = collapsedTextFanIn;
    resolvedInputs['Artifact:ScriptProducer.SegmentText[0]'] = 'First segment';
    resolvedInputs['Artifact:ScriptProducer.SegmentText[1]'] = 'Second segment';
    ...
    expect(firstClipEffects).toHaveLength(1);
    expect(firstClipEffects?.[0]?.text).toBe('First segment');
    expect(firstClipEffects?.[0]?.startTime).toBe(0);
    expect(firstClipEffects?.[0]?.duration).toBeCloseTo(12);
    ...
    expect(secondClipEffects).toHaveLength(1);
    expect(secondClipEffects?.[0]?.text).toBe('Second segment');
    expect(secondClipEffects?.[0]?.startTime).toBeCloseTo(12);
    expect(secondClipEffects?.[0]?.duration).toBeCloseTo(8);
  });

  it('supports textClip shorthand with configured transition', async () => {
    const handler = createHandler();
    const request = makeRequest();
    const resolvedInputs = request.context.extras?.resolvedInputs as Record<
      string,
    ...
    config.config.timeline = {
      ...
      tracks: ['Audio', 'Text'],
      audioClip: { artifact: 'AudioSegments' },
      textClip: { artifact: 'TextSegments', effect: 'slide-in-out-left' },
    };
    request.inputs.push('Input:TimelineComposer.TextSegments');
    ...
    const textFanIn = {
      groupBy: 'segment',
      groups: [
        ['Artifact:TextSegments[0][0]'],
        ['Artifact:TextSegments[1][0]'],
      ],
    };
    resolvedInputs['Input:TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['Artifact:TextSegments[0][0]'] = 'Segment one';
    resolvedInputs['Artifact:TextSegments[1][0]'] = 'Segment two';
    ...
    expect(firstClipEffects?.[0]?.transition).toBe('slide-in-out-left');
  });

  it('throws when text snippet payload is not a string', async () => {
    const handler = createHandler();
    const request = makeRequest();
    const resolvedInputs = request.context.extras?.resolvedInputs as Record<
      string,
    ...
    const config = request.context.providerConfig as {
      config: {
        timeline: { clips: Array<Record<string, unknown>>; tracks: string[] };
      };
    };
    ...
    const textFanIn = {
      groupBy: 'segment',
      groups: [['Artifact:TextSegments[0][0]'], []],
    };
    resolvedInputs['Input:TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['Artifact:TextSegments[0][0]'] = createAssetPayload(3);

    await expect(handler.invoke(request)).rejects.toThrow(
      /must resolve to a string payload/i
    );
  });

  it('throws when text effect is unsupported', async () => {
    const handler = createHandler();
    const request = makeRequest();
    const resolvedInputs = request.context.extras?.resolvedInputs as Record<
      string,
    ...
    resolvedInputs['Input:TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['TimelineComposer.TextSegments'] = textFanIn;
    resolvedInputs['Artifact:TextSegments[0][0]'] = 'Valid snippet';

    await expect(handler.invoke(request)).rejects.toThrow(
      /Unsupported Text clip effect/i
    );
  });
  ...
  describe('master track item expansion', () => {
    ...
    it('spans non-master audio across expanded segments', async () => {
      ...
      const videoGroups = [['Artifact:Video[0][a]', 'Artifact:Video[0][b]']];
      resolvedInputs['Input:TimelineComposer.VideoSegments'] = {
        groupBy: 'segment',
        groups: videoGroups,
      };
      ...
      const audioGroups = [['Artifact:Audio[0]']];
      resolvedInputs['Input:TimelineComposer.AudioSegments'] = {
        groupBy: 'segment',
        groups: audioGroups,
      };
      ...
      expect(audioTrack?.clips[0]?.properties.assetId).toBe(
        'Artifact:Audio[0]'
      );
    });
    ...
    it('expands primary audio clips per segment when a group has multiple assets', async () => {
      ...
      expect(audioTrack?.clips[1]?.properties.assetId).toBe(
        'Artifact:Audio[0][b]'
      );
      expect(audioTrack?.clips[1]?.startTime).toBeCloseTo(4);
      expect(audioTrack?.clips[1]?.duration).toBeCloseTo(6);
      expect(audioTrack?.clips[2]?.properties.assetId).toBe(
        'Artifact:Audio[1]'
      );
    ...
    it('uses expanded segment count for Duration fallback when primary master has no native durations', async () => {
      ...
      const imageTrack = timeline.tracks.find(
        (track) => track.kind === 'Image'
      );
      expect(imageTrack).toBeDefined();
    ...
    it('repeats images for each expanded segment from the same group', async () => {
      ...
      const imageGroups = [['Artifact:Image[0][0]', 'Artifact:Image[0][1]']];
      resolvedInputs['Input:TimelineComposer.ImageSegments'] = {
        groupBy: 'segment',
        orderBy: 'image',
        groups: imageGroups,
      ...
      const timeline = JSON.parse(
        ...
        tracks: Array<{
          ...
          clips: Array<{
            startTime: number;
            duration: number;
            properties: { effects?: Array<{ assetId: string }> };
          }>;
        }>;
      };
      ...
      expect(
        imageTrack?.clips[0]?.properties.effects?.map((e) => e.assetId)
      ).toEqual(['Artifact:Image[0][0]', 'Artifact:Image[0][1]']);
      expect(
        imageTrack?.clips[1]?.properties.effects?.map((e) => e.assetId)
      ).toEqual(['Artifact:Image[0][0]', 'Artifact:Image[0][1]']);
    ...
    it('handles mixed groups with varying item counts', async () => {
      ...
      expect(videoTrack?.clips[3]?.properties.assetId).toBe(
        'Artifact:Video[2]'
      );
